...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var ytick = otuIDs.slice(0,10).reverse().map(function (elem) {return `OTU ${elem}`});
    var xtick = sampleVals.slice(0,10).reverse();
    ...
    var gaugeData = {
      ...
      gauge: {
        ...
        steps: [
          ...
          {range: [2,4], color:"#FFB266"},
          {range: [4,6], color:"#FFFF00"},
          {range: [6,8], color:"#80FF00"},
          {range: [8,10], color:"#00FF80"}
        ]
      }
    };
